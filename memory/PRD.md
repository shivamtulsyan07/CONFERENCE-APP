...
- 2026-06 Code-review pass: applied the genuine findings — stable React list keys on the dashboard tables and Haana messages/chips, `load` on Dashboard and `refreshTotals` on Shop Sale wrapped in `useCallback` (with an `fRef` safety net) so effects have honest dependencies and fire exactly once per change. Deliberately NOT changed: the "possibly undefined `r`/`k`" Python findings (generator-expression locals, false positives), the localStorage finding (only a random chat session id, no credentials), and the large complexity refactors of `useSheet.js` / `filters.js` / `LineSheet.jsx` / `dashboard_overview()` (high regression risk on a live app, no user-visible benefit). Verified: iteration_16 (backend 91/91, frontend all checks, dashboard render-loop check 0 idle calls, 0 duplicate-key warnings).
